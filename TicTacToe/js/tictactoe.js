//this function is fired once the DOM is loaded.
window.onload = function() {watch()};
function watch() {
    var btn = document.getElementById("btnStop");
    btnDisabled(btn); //disable the sto button since the game hasn't started
}

//this function will roll a random number twice
// once for each player, to decide who goes first
function rollForTurn() {
    ...
    // build the string toshow which player rolled what die roll
    for (i = 0; i < xArray.length; i++) {
        var result = i + 1;
        var pOne = xArray[0];
        var pTwo = xArray[1];
        if (pOne == pTwo) { //riging roll on tie to avoid bug in code, need to address later
            pOne = 1;
            pTwo = 2;
        }
...
function startGame() {
    ...
    btnDisabled(btn);  //disable the start button since the game has now started
    var btn = document.getElementById('btnStop');
...
function btnDisabled(btn) {
    ...
    btn.disabled = true; //.diabled automatically diables an html button
}
...
function startEnabled(btn) {
    ...
    btn.style.ackgroundColor = "rgb(57, 230, 0)";
    btn.disabled = false;
}
...
//thisfunction will conceal the message console from view
...
function saveSettings() {
    var p1Index = document.getElementById("player1").selectedIndex;  //selectedIndex is whichever option is selected by the user
    var p1Selected = document.getElementById("player1").options; //within the select tags in html, there are two "options"
...
function recordMoves(square) {
    ...
    verdict = check(info, square); // vall funtion to check if proposed square is already occupied
    return verdict;
}
...
function check4Tie() {
    ...
    boardState = boardState.split(','); // remove the string by commas into an array
...
function winner(winDetected, winCon) {
    if (winDetected == "win") {
        ...
        writeMsg(txt2);
        var btn = document.getElementById('btnStart');
        startEnabled(btn); //enable the start button wince the game is now stopped
        var btn = document.getElementById('btnStop');
        btnDisabled(btn); // disable the stop button since the game is now stopped
...
// this function will make the winning squares liht up in celebration
...
// flash for a few seconds or a win animation
function blink() {
...
function square1Animate() {
    var activePlayer = document.getElementById('showPlayer').innerHTML;
    if (activePlayer != "Game Stopped") { // if game as not yet started prevent avatar placement
...
//this functin will perform the animation for the O avatar.
